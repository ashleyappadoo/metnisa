...
const EXECUTOR_TOKEN_SHA256 =
  "55b11552ead03766c7c0a3f55c16ca3a3a6215c80d5615605d2c848fa0094bb1";
...
async function getArtworkGeometry(frontImage: TemplateImage) {
  ...
  if (frontImage.src) {
    ...
    try {
      contentHeight = (await measureArtwork(sourceBuffer)).height;
    } catch {
      contentHeight = 0;
    }
  }
...
export async function GET(request: Request) {
  try {
    ...
    if (
      ...
      url.searchParams.get("confirm") !== "DROP001" ||
    ...
    const template = (await getPrintifyProduct(
      shopId,
      templateProductId,
    )) as unknown as TemplateProduct;
    ...
    for (const product of DROP_PRODUCTS) {
      ...
      const uploaded = await uploadPrintifyImage(product.fileName, imageBuffer.toString("base64"));
      const variants = variantsForEdition(template, product.edition, whiteIds);
      if (!variants.some((variant) => variant.is_enabled)) {
